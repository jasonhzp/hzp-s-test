...
| 登记后 3 个工作日内 | 首次触达客户,介绍前置培训、**确认是否需要培训**(需要→建群;不需要→标记【无需培训】归档;无响应→三次触达) | 按产品型号备齐培训材料 |
| 客户**确认需要**培训后 2 个工作日内 | 建 WhatsApp 群,拉售后入群并介绍 | 进群后**直接与客户敲定培训时间**,发 Zoom 链接(同步工厂对接人) |
...
flowchart TD
    ...
    D --> E["业务员:首次触达客户<br>介绍免费培训 + 确认是否需要培训"]
    E --> F{"客户需要培训吗?"}
    F -- "3 次触达无响应" --> G["标记【挂起】<br>到港/清关节点再确认"]
    G --> E
    F -- "不需要培训" --> X["登记表标记【无需培训】<br>发操作卡、归档,不建群(终态)"]
    F -- "需要培训" --> H["业务员建 WhatsApp 群<br>拉售后 + 客户 + 客户工程师"]
...
### 5.3 第二步:首次触达客户、确认培训需求(登记后 3 个工作日内)
...
这一步的核心是**确认客户到底需不需要培训**——不是每个客户都需要。按客户反应分三种结果处理:

| 客户反应 | 处理 |
| --- | --- |
| **需要培训** | 进入 5.4:客户同意后 2 个工作日内建群,把售后介绍给客户 |
| **明确不需要培训** | 登记表标记 **【无需培训】**,发一份基础操作卡备用,任务**归档、不建群**;货到时再问候一次,留个口子。跟进表状态置为**无需培训**(终态) |
| **无响应** | 走下方三次触达原则 |

> 这一步只需拿到"**是否需要培训 + 谁来参加**"两点,**不用和客户敲具体培训时间**;时间由售后建群后直接和客户约。
...
- 三次无响应 → 在登记表备注"客户暂无响应",通知售后把跟进表状态改为**挂起**。挂起不等于放弃:**货物到港/清关是客户必然出现的节点**,届时业务员必须再触达一次、再确认一次是否需要培训。

### 5.4 第三步:建群建联(**确认客户需要培训后**,客户同意后 2 个工作日内)

> 前提:仅对**确认需要培训**的客户建群。不需要培训的客户已在 5.3 归档,不进入本步。
...
**另一种终态——无需培训·已归档**:客户在 5.3 明确表示不需要培训的,登记表标记【无需培训】、发基础操作卡备用、任务归档,即为该单终态,不再要求走培训四条件。每条售后前置任务最终都必须落在【已闭环】或【无需培训】之一,不允许静默消失。
